...
public class SeeTabButton extends JButton implements MouseListener {
	private String name;
	private Image img;
//	private boolean isCliked = false;
	...
	SeeTabButton() {
	    ...
	    try {
	      img = ImageIO.read(new File("img/restore.png"));
	    } catch (IOException e) {
	      e.printStackTrace();
	    }
...
//		  if (getCliked == false)
			  try {
				  this.img = ImageIO.read(new File("img/restore.png"));
				  } catch (IOException e) {
					  e.printStackTrace();
					  }
	  	}
